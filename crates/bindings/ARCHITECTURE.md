...
- JS API (`crates/js_api`):
  - Builds order builder calldata for approvals/deposits/add‑order and uses call structs like `Raindex::multicallCall` without needing on‑chain RPC instance helpers.
- CLI (`crates/cli`):
