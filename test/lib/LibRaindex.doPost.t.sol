...
/// Tests for `LibRaindex.doPost` behavior via the `entask2` entry point.
contract LibRaindexDoPostTest is RaindexV6ExternalRealTest {
    /// Empty post array is a no-op.
    function testDoPostEmptyArray() external {
        TaskV2[] memory emptyTasks = new TaskV2[](0);
        iRaindex.entask2(emptyTasks);
    }
