...
contract LibRaindexArbFinalizeArbFuzzTest is Test {
    ...
    /// hold the rest.
    function testFinalizeArbFuzz(uint256 raindexOutputAmount, uint256 swapAmount, uint256 raindexPullAmount) external {
        // Bound to avoid zero-amount edge cases and overflow.
        raindexOutputAmount = bound(raindexOutputAmount, 1, 1e30);
        swapAmount = bound(swapAmount, 1, raindexOutputAmount);
        raindexPullAmount = bound(raindexPullAmount, 1, swapAmount);
        ...
        ArbResult memory result = LibTestArb.setupAndArb(
            vm, raindexPullAmount, raindexOutputAmount, exchangeInputAmount, swapAmount, LibTestArb.noopTask(), 0
        );
        ...
        // Input token profit = swapAmount - raindexPullAmount.
        uint256 expectedInputProfit = swapAmount - raindexPullAmount;
        assertEq(result.inputToken.balanceOf(address(this)), expectedInputProfit, "sender inputToken profit");

        // Output token profit = raindexOutputAmount - swapAmount.
        uint256 expectedOutputProfit = raindexOutputAmount - swapAmount;
        assertEq(result.outputToken.balanceOf(address(this)), expectedOutputProfit, "sender outputToken profit");

        // OB got what it pulled.
        assertEq(result.inputToken.balanceOf(address(result.raindex)), raindexPullAmount, "OB inputToken");
