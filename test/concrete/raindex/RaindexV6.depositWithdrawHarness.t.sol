...
contract RaindexV6DepositWithdrawHarnessTest is Test, IRaindexV6Stub {
    ...
    /// and their approval to Raindex (the most Raindex could ever pull). When the
    /// APPROVAL is the smaller of the two, the approval is returned, not the
    /// balance. Exercised against a REAL ERC20 (real balanceOf/allowance) so a
    /// mutation returning the balance, or `max(...)`, is caught. (The mock-based
    /// vaultBalanceVault0 suite asserts the same selection but runs against the
