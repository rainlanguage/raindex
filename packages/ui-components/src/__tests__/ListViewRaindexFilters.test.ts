...
const { mockPageStore } = await vi.hoisted(
  () => import("$lib/__mocks__/stores.ts"),
);
...
describe("ListViewRaindexFilters", () => {
  ...
  test("shows vault-specific components on vault page", () => {
    ...
    expect(
      screen.getByTestId("zero-balance-vault-checkbox"),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.queryByTestId("order-status-checkbox"),
  ...
  test("shows order-specific components on orders page", () => {
    ...
    expect(
      screen.queryByTestId("zero-balance-vault-checkbox"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("inactive-orders-vault-checkbox"),
    ).not.toBeInTheDocument();
  });
  ...
  test("shows raindexes dropdown when raindexes exist", () => {
    ...
    expect(
      screen.getByTestId("dropdown-raindexes-filter-button"),
    ).toBeInTheDocument();
  });
