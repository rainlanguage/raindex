...
describe("TokenInput", () => {
  ...
  it("calls setVaultId when input changes", async () => {
    const input = render(TokenIOInput, mockProps).getByPlaceholderText(
      "Enter vault ID",
    );
    await fireEvent.input(input, { target: { value: "vault1" } });
    expect(builderInstance.setVaultId).toHaveBeenCalledWith(
      "input",
      "test",
      "vault1",
    );
  ...
  it("calls setVaultId on output vault when input changes", async () => {
    const input = render(TokenIOInput, outputMockProps).getByPlaceholderText(
      "Enter vault ID",
    );
