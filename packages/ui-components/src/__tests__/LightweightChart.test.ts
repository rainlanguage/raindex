...
test("renders without data correctly", async () => {
  ...
  await waitFor(() => {
    expect(screen.getByTestId("lightweightChartTitle")).toHaveTextContent(
      title,
    );
  ...
  await waitFor(() => {
    expect(
      screen.getByTestId("lightweightChartEmptyMessage"),
    ).toHaveTextContent(emptyMessage);
  });
});
...
test("renders with data correctly", async () => {
  ...
  await waitFor(() => {
    ...
    expect(
      screen.queryByTestId("lightweightChartYearButtons"),
    ).toBeInTheDocument();
