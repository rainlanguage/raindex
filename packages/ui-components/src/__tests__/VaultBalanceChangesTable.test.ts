...
const createMockVaultBalanceChange = (
  ...
  const type =
    (overrides.type as RaindexVaultBalanceChangeType) || "withdrawal";
  return {
    type,
...
test("renders the vault list table with correct data", async () => {
  ...
  const mockVaultBalanceChanges: RaindexVaultBalanceChange[] = [
    createMockVaultBalanceChange(),
  ];
  const mockVault: RaindexVault = {
    id: "vault1",
    getBalanceChanges: vi
      .fn()
      .mockResolvedValue({ value: mockVaultBalanceChanges }),
...
test("it shows the correct data in the table with combined Info column", async () => {
  ...
  await waitFor(() => {
    ...
    expect(infoCell).toHaveTextContent(
    ...
    const balanceChangeCell = screen.getByTestId(
