...
describe("VaultDetail", () => {
  ...
  it("shows the correct data when the query returns data", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("vaultDetailTokenName")).toHaveTextContent(
        "USDC coin",
      );
      expect(screen.getByTestId("vaultDetailVaultId")).toHaveTextContent(
        "Vault ID 0x3e8",
      );
      ...
      expect(screen.getByTestId("vaultDetailBalance")).toHaveTextContent(
        "Balance 100000 USDC",
      );
      expect(
        screen.queryByTestId("vaultDetailOrdersAsInput"),
      ).toHaveTextContent("None");
      expect(
        screen.queryByTestId("vaultDetailOrdersAsOutput"),
      ).toHaveTextContent("None");
    });
  });
  ...
  it("doesn't show deposit/withdraw buttons when account doesn't match", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("depositOrWithdrawButton"),
  ...
  it("failed query invalidation triggers a toast", async () => {
    ...
    (invalidateTanstackQueries as Mock).mockRejectedValue(
