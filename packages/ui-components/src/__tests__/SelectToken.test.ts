...
describe("SelectToken", () => {
  ...
  it("calls setSelectToken and updates token info when input changes", async () => {
    ...
    await waitFor(() => {
      expect(mockBuilderWithNoToken.setSelectToken).toHaveBeenCalledWith(
        "input",
        "0x456",
      );
    });
  ...
  describe("Dropdown Mode", () => {
    ...
    it("switches to custom mode when custom button is clicked", async () => {
      ...
      expect(screen.getByTestId("dropdown-mode-button")).not.toHaveClass(
        "border-blue-300",
      );
    });
    ...
    it("shows custom input in custom mode", async () => {
      ...
      expect(
        screen.getByPlaceholderText("Enter token address (0x...)"),
      ).toBeInTheDocument();
    });
    ...
    it("displays selected token info when token is selected", async () => {
      ...
      expect(
        screen.getByTestId(`select-token-success-${mockProps.token.key}`),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Balance Display", () => {
    ...
    it("shows loading spinner when balance is loading", async () => {
      ...
      const tokenStatus = screen.getByTestId(
        `select-token-success-${mockProps.token.key}`,
      );
