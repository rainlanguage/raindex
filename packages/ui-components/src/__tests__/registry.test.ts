...
file2.js https://example.com/file2.js`;
  ...
  it("should handle failed fetch response", async () => {
    ...
    await expect(
      fetchParseRegistry("https://example.com/registry"),
...
file2.rain https://example.com/file2.rain`;
  ...
  it("should handle failed dotrain fetch", async () => {
    ...
    await expect(
      fetchRegistryDotrains("https://example.com/registry"),
    ).rejects.toThrow("Failed to fetch dotrain for file1.rain");
  });
  ...
  it("should handle network errors during dotrain fetch", async () => {
    ...
    await expect(
      fetchRegistryDotrains("https://example.com/registry"),
    ).rejects.toThrow("Error fetching dotrain for file1.rain: Network error");
  });
});
...
describe("validateOrders", () => {
  ...
  it("should validate orders and categorize them properly", async () => {
    ...
    expect(RaindexOrderBuilder.getOrderDetails).toHaveBeenCalledWith(
      "valid dotrain content",
    );
    expect(RaindexOrderBuilder.getOrderDetails).toHaveBeenCalledWith(
      "invalid dotrain content",
    );
    expect(RaindexOrderBuilder.getOrderDetails).toHaveBeenCalledWith(
